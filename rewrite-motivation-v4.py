...
def quote(text, attribution=None):
    attr = f'<p style="font-size:0.82em;font-weight:700;color:#DC373E;margin:14px 0 0;letter-spacing:0.05em;text-transform:uppercase;">&mdash; {attribution}</p>' if attribution else ''
    return (
        ...
        f'position:relative;z-index:1;">{text}</p>'
        f'{attr.replace("color:#DC373E", "color:#f87171") if attr else ""}'
        '</div>'
    )
...
def tip(text):
    return (
        ...
        'border:1px solid #6ee7b7;border-radius:14px;padding:18px 22px;margin:28px 0;'
        'display:flex;gap:14px;align-items:flex-start;'
        'box-shadow:0 2px 12px rgba(16,185,129,0.1);">'
        '<div style="background:#10b981;border-radius:8px;padding:6px 8px;flex-shrink:0;margin-top:2px;">'
        '<span style="font-size:1em;line-height:1;">&#128161;</span></div>'
        '<div><p style="font-size:0.76em;font-weight:800;color:#065f46;text-transform:uppercase;'
        'letter-spacing:0.08em;margin:0 0 5px;">Quick Tip</p>'
        f'<p style="font-size:0.93em;color:#064e3b;margin:0;line-height:1.65;">{text}</p>'
        '</div></div>'
    )

def section_header(num, title):
    return (
        f'<div style="display:flex;align-items:center;gap:16px;margin:48px 0 20px;">'
        f'<div style="background:linear-gradient(135deg,#DC373E,#b91c1c);color:#fff;font-size:1em;'
        f'font-weight:800;width:42px;height:42px;border-radius:12px;display:flex;align-items:center;'
        f'justify-content:center;flex-shrink:0;box-shadow:0 4px 12px rgba(220,55,62,0.35);">{num}</div>'
        f'<h2 style="font-size:1.6em;font-weight:800;color:#0F3154;margin:0;line-height:1.25;">{title}</h2>'
        f'</div>'
    )

def divider():
    return (
        '<div style="display:flex;align-items:center;gap:12px;margin:40px 0;">'
...
'<p style="font-size:0.85em;font-weight:700;color:#DC373E;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:10px;display:inline-block;background:#FFF1F2;padding:4px 12px;border-radius:20px;">Parent Guide &amp; Mindset</p>\n\n'
'<p class="lead" style="font-size:1.18em;line-height:1.8;color:#374151;">It was a Tuesday evening, and I was standing in the backyard holding a soccer ball, calling Matthew\'s name for the third time. No answer. I walked inside and found him exactly where I expected — on the couch, controller in hand, halfway through some game I\'d never heard of. He glanced up at me with that look every soccer parent knows: <em>please don\'t make me go outside.</em></p>\n\n'
...
+ '<div style="background:linear-gradient(135deg,#EEF4FF 0%,#E8F0FE 100%);border-radius:20px;padding:36px;margin:0 0 48px;border:1px solid #C7D7FA;">'
'\n  <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">'
'\n    <span style="font-size:1.3em;">&#127911;</span>'
'\n    <p style="font-size:0.75em;font-weight:800;color:#DC373E;text-transform:uppercase;letter-spacing:0.1em;margin:0;">From the Podcast</p>'
'\n  </div>'
'\n  <h2 style="font-size:1.45em;font-weight:800;color:#0F3154;margin:0 0 4px;line-height:1.3;">Neil\'s Framework for Lasting Motivation</h2>'
'\n  <p style="font-size:0.85em;color:#64748b;margin:0 0 22px;font-style:italic;">Listen to the full episode of <em>The Inside Scoop</em>, then read the framework below.</p>'
'\n  <iframe data-testid="embed-iframe" style="border-radius:12px;display:block;" src="https://open.spotify.com/embed/episode/3nYh9J85TXJfpMO9cVCiEC?utm_source=generator" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>'
'\n</div>\n\n'
...
'<p>This is also why "just make it fun" gets you only so far. Fun solves the immediate resistance — it makes today feel good. But motivation that lasts is built when your child can <em>see the result</em> of what they\'re doing. Fun is the entry point. Visible progress is what keeps them coming back.</p>\n\n'
...
'<p>When Adam hit a wall around age ten, it wasn\'t laziness. He\'d been putting in real work but not seeing the results he expected. His friends seemed to be improving faster (they probably weren\'t, but perception is everything at that age). He was quietly frustrated, and that frustration was coming out as resistance.</p>\n\n'
...
+ '<p>Not "you\'re so talented." Not "you\'re the best on your team." Specific. Observable. Tied to effort, not identity. That\'s the sentence that builds growth mindset. Kids who believe improvement is real will practice. Kids who don\'t, won\'t. No amount of fun closes that gap.</p>\n\n'
...
+ '<p>This is the part that\'s hard to explain to a parent whose kid is begging to shoot on goal: letting them do what\'s fun in the short term at the expense of what builds the fastest visible progress actually works against long-term motivation. The irony is real. If you want structured ball mastery progressions, <a href="https://www.anytime-soccer.com/our-picks" style="color:#DC373E;font-weight:600;text-decoration:underline;text-underline-offset:3px;">our recommended training tools</a> are a good place to start.</p>\n\n'
+ tip("Start with 10 minutes of pure ball mastery before anything else — even if they\'d rather shoot. The feet get sharp, confidence builds, and they usually want to keep going. Momentum is everything.")
+ divider()

+ section_header("4", "Autonomy: Real Choices, Not False Ones")
+ '<p>There\'s a version of "giving choices" that backfires: false choices. "Do you want to practice today — yes or no?" isn\'t a real choice. It\'s a question with a wrong answer, and kids know it immediately.</p>\n\n'
...
+ '<p>The shift that worked for me: participating instead of coaching from the sideline. When I laughed at my own bad juggling attempts, the whole dynamic changed. He stopped feeling evaluated. And somewhere along the way, he stopped needing to be invited outside at all. You don\'t need to be a skilled player to train alongside your child. You just need to show up as a participant, not a spectator.</p>\n\n'
...
'<p style="font-size:0.78em;font-weight:800;color:#0F3154;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 14px;display:flex;align-items:center;gap:8px;"><span style="background:#0F3154;color:#fff;width:22px;height:22px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:0.85em;flex-shrink:0;">&#x23F0;</span>Before You Go Outside</p>'
...
+ '<div style="background:#F8FAFC;border:1px solid #E1E8EF;border-radius:16px;padding:24px;">'
'<p style="font-size:0.78em;font-weight:800;color:#0F3154;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 14px;display:flex;align-items:center;gap:8px;"><span style="background:#DC373E;color:#fff;width:22px;height:22px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:0.85em;flex-shrink:0;">&#9917;</span>During the Session</p>'
'<ul style="padding-left:18px;line-height:2;color:#374151;font-size:0.9em;margin:0;">'
'<li>More praise than corrections?</li>'
'<li>Participating, not just watching?</li>'
'<li>At least one free-play window?</li>'
'<li>One specific thing called out as well done?</li>'
'</ul></div>'

+ '<div style="background:#F8FAFC;border:1px solid #E1E8EF;border-radius:16px;padding:24px;">'
'<p style="font-size:0.78em;font-weight:800;color:#0F3154;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 14px;display:flex;align-items:center;gap:8px;"><span style="background:#16a34a;color:#fff;width:22px;height:22px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:0.85em;flex-shrink:0;">&#10003;</span>After the Session</p>'
'<ul style="padding-left:18px;line-height:2;color:#374151;font-size:0.9em;margin:0;">'
'<li>Ended on a high note?</li>'
'<li>Skipped the post-session critique?</li>'
'<li>Expressed genuine enjoyment — not just pride?</li>'
'</ul></div>'
...
'<p style="font-size:0.9em;color:#7f1d1d;line-height:1.7;margin:0 0 12px;"><strong>Correct every mistake.</strong> Max one thing per session. Let everything else go.</p>'
...
'<p style="font-size:0.9em;color:#7f1d1d;line-height:1.7;margin:0;"><strong>Push through genuine resistance.</strong> Back off. Have a conversation. The resistance is usually about something else.</p>'
'</div>'
...
+ '\n<div style="background:linear-gradient(135deg,#0F3154 0%,#1a4a78 100%);border-radius:20px;padding:40px;margin:48px 0;text-align:center;box-shadow:0 8px 32px rgba(15,49,84,0.25);">'
'<p style="font-size:0.75em;font-weight:800;color:rgba(255,255,255,0.5);text-transform:uppercase;letter-spacing:0.12em;margin:0 0 10px;">Ready to Make It Easier?</p>'
'<h3 style="font-size:1.55em;font-weight:800;color:#ffffff;margin:0 0 14px;line-height:1.3;">Build the Habit With a Plan That Does the Thinking For You</h3>'
'<p style="color:rgba(255,255,255,0.7);margin:0 0 28px;font-size:0.95em;max-width:480px;margin-left:auto;margin-right:auto;">Thousands of drill videos organized by age and skill level. You show up. We handle the curriculum.</p>'
'<a href="https://app.anytime-soccer.com/register" style="display:inline-block;background:linear-gradient(135deg,#DC373E,#b91c1c);color:#ffffff;font-weight:800;font-size:1em;padding:16px 36px;border-radius:12px;text-decoration:none;box-shadow:0 4px 16px rgba(220,55,62,0.5);letter-spacing:0.02em;">Start Your Free 7-Day Trial &#8594;</a>'
'</div>\n\n'

f'<div style="display:flex;align-items:flex-start;gap:20px;background:linear-gradient(135deg,#F8FAFC,#EEF2FF);border-radius:16px;padding:24px 28px;margin-top:40px;border:1px solid #C7D7FA;box-shadow:0 2px 12px rgba(15,49,84,0.06);">'
f'<img src="{AVATAR}" alt="Neil Crawford" style="width:68px;height:68px;border-radius:50%;object-fit:cover;flex-shrink:0;border:3px solid #fff;box-shadow:0 4px 14px rgba(15,49,84,0.2);" />'
'<div>'
'<p style="font-size:0.75em;font-weight:800;color:#DC373E;text-transform:uppercase;letter-spacing:0.09em;margin:0 0 4px;">About the Author</p>'
