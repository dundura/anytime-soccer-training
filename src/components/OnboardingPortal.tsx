...
const WORKFLOW_GROUPS: { title: string; keys: string[]; reference?: boolean; sub?: { title: string; keys: string[] }[] }[] = [
  ...
  { title: 'FAQs', reference: true, keys: REFERENCE_KEYS,
    ...
    sub: [
      ...
      { title: 'Managing Team & Roster', keys: ['faq_delete_team', 'faq_invite_players', 'faq_add_remove_players', 'faq_videos_move', 'faq_managers'] },
    ] },
];
...
const UPGRADING_NEW_TEAM_SECTIONS: StepSection[] = [
  ...
  // applying-a-slot steps come after either one rather than under Method Two.
  {
    ...
    overview: 'Two options &mdash; you pay online and we add <strong>free access slots</strong> to your account, or you buy them in the app &mdash; you always need to <strong>apply a free slot to each player&rsquo;s account</strong>.',
    items: [
...
export default function OnboardingPortal() {
  ...
  const submitAuth = async () => {
    ...
    try {
      ...
      setTeamNameInput(data.coach.teamName || '');
      setWizardIndex(firstIncomplete(data.coach));
      // No route question. A coach is here to onboard their team; the club
      // route is a separate link sent to a director, and asking every coach
      // which of the two they are was a decision they never needed to make.
    } catch {
  ...
  // Set while an admin is looking at somebody else's portal. The CRM opens it
  // from the console; this banner and the way back live here, because here is
  // where you land.
